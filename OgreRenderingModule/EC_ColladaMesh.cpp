...
DEFINE_POCO_LOGGING_FUNCTIONS("EC_ColladaMesh")

#include "MemoryLeakCheck.h"

using namespace OgreRenderer;

EC_ColladaMesh::EC_ColladaMesh(IModule* module) :
    ...
    attached_to_bone_(false)
{
    ...
    adjustment_node_ = scene_mgr->createSceneNode(renderer->GetUniqueObjectName("EC_ColladaMesh_adjustment_node"));
...
void EC_ColladaMesh::SetPlaceable(ComponentPtr placeable)
{
...
void EC_ColladaMesh::SetPlaceable(EC_Placeable* placeable)
{
...
void EC_ColladaMesh::View(const QString &attributeName)
{
    // todo add implementation.
}

void EC_ColladaMesh::AutoSetPlaceable()
{
...
void EC_ColladaMesh::SetAdjustPosition(const Vector3df& position)
{
...
void EC_ColladaMesh::SetAdjustOrientation(const Quaternion& orientation)
{
    Transform transform = nodeTransformation.Get();
    Vector3df euler;
...
void EC_ColladaMesh::SetAdjustScale(const Vector3df& scale)
{
    Transform transform = nodeTransformation.Get();
...
void EC_ColladaMesh::SetAttachmentPosition(uint index, const Vector3df& position)
{
...
void EC_ColladaMesh::SetAttachmentOrientation(uint index, const Quaternion& orientation)
{
    if (index >= attachment_nodes_.size() || attachment_nodes_[index] == 0)
        return;
...
void EC_ColladaMesh::SetAttachmentScale(uint index, const Vector3df& scale)
{
    if (index >= attachment_nodes_.size() || attachment_nodes_[index] == 0)
        return;
...
Vector3df EC_ColladaMesh::GetAdjustPosition() const
{
    Transform transform = nodeTransformation.Get();
    return transform.position;
}

Quaternion EC_ColladaMesh::GetAdjustOrientation() const
{
    Transform transform = nodeTransformation.Get();
...
Vector3df EC_ColladaMesh::GetAdjustScale() const
{
    Transform transform = nodeTransformation.Get();
    return transform.scale;
}

Vector3df EC_ColladaMesh::GetAttachmentPosition(uint index) const
{
    if (index >= attachment_nodes_.size() || attachment_nodes_[index] == 0)
...
Quaternion EC_ColladaMesh::GetAttachmentOrientation(uint index) const
{
    if (index >= attachment_nodes_.size() || attachment_nodes_[index] == 0)
        return Quaternion();
...
Vector3df EC_ColladaMesh::GetAttachmentScale(uint index) const
{
    if (index >= attachment_nodes_.size() || attachment_nodes_[index] == 0)
...
void EC_ColladaMesh::SetDrawDistance(float draw_distance)
{
    drawDistance.Set(draw_distance, AttributeChange::LocalOnly);
}

bool EC_ColladaMesh::SetMesh(QString meshResourceName, bool clone)
{
    if (!ViewEnabled())
        return false;
    ...
    try
    {
        entity_ = scene_mgr->createEntity(renderer->GetUniqueObjectName("EC_ColladaMesh_entity"), mesh->getName());
        if (!entity_)
        {
...
bool EC_ColladaMesh::SetMeshWithSkeleton(const std::string& mesh_name, const std::string& skeleton_name, bool clone)
{
    if (!ViewEnabled())
        return false;
    if (renderer_.expired())
...
//        LogDebug("Set skeleton " + skeleton_name + " to mesh " + mesh_name);
    ...
    try
    {
        entity_ = scene_mgr->createEntity(renderer->GetUniqueObjectName("EC_ColladaMesh_entwithskel"), mesh->getName());
        if (!entity_)
        {
            LogError("Could not set mesh " + mesh_name);
            return false;
...
void EC_ColladaMesh::RemoveMesh()
{
    if (renderer_.expired())
        return;
    RendererPtr renderer = renderer_.lock();
...
bool EC_ColladaMesh::SetAttachmentMesh(uint index, const std::string& mesh_name, const std::string& attach_point, bool share_skeleton)
{
    if (!ViewEnabled())
        return false;
    if (renderer_.expired())
    ...
    try
    {
        QString entityName = QString("EC_ColladaMesh_attach") + QString::number(index);
        ...
        {
            QString nodeName = QString("EC_ColladaMesh_attachment_") + QString::number(index);
...
void EC_ColladaMesh::RemoveAttachmentMesh(uint index)
{
    if (renderer_.expired())
        return;
    RendererPtr renderer = renderer_.lock();
...
void EC_ColladaMesh::RemoveAllAttachments()
{
...
bool EC_ColladaMesh::SetMaterial(uint index, const std::string& material_name)
{
    if (!entity_)
    {
...
bool EC_ColladaMesh::SetMaterial(uint index, const QString& material_name)
{
    return SetMaterial(index, material_name.toStdString());
}

bool EC_ColladaMesh::SetAttachmentMaterial(uint index, uint submesh_index, const std::string& material_name)
{
...
void EC_ColladaMesh::SetCastShadows(bool enabled)
{
    castShadows.Set(enabled, AttributeChange::LocalOnly);
}

uint EC_ColladaMesh::GetNumMaterials() const
{
    if (!entity_)
        return 0;
...
uint EC_ColladaMesh::GetAttachmentNumMaterials(uint index) const
{
    if (index >= attachment_entities_.size() || attachment_entities_[index] == 0)
        return 0;
...
const std::string& EC_ColladaMesh::GetMaterialName(uint index) const
{
...
const QString& EC_ColladaMesh::GetMatName(uint index) const
{
    const QString &name(GetMaterialName(index).c_str());
    return name;
}

const std::string& EC_ColladaMesh::GetAttachmentMaterialName(uint index, uint submesh_index) const
{
    const static std::string empty;
...
bool EC_ColladaMesh::HasAttachmentMesh(uint index) const
{
    if (index >= attachment_entities_.size() || attachment_entities_[index] == 0)
        return false;
...
Ogre::Entity* EC_ColladaMesh::GetAttachmentEntity(uint index) const
{
    if (index >= attachment_entities_.size())
        return 0;
...
const uint EC_ColladaMesh::GetNumSubMeshes() const
{
    uint count = 0;
    if (HasMesh())
...
const std::string& EC_ColladaMesh::GetMeshName() const
{
...
const std::string& EC_ColladaMesh::GetSkeletonName() const
{
    static std::string empty_name;
...
void EC_ColladaMesh::GetBoundingBox(Vector3df& min, Vector3df& max) const
{
    if (!entity_)
    {
...
QVector3D EC_ColladaMesh::GetWorldSize() const
{
...
void EC_ColladaMesh::DetachEntity()
{
    if (attached_to_bone_)
        return;
...
void EC_ColladaMesh::AttachEntity()
{
    if (attached_to_bone_)
        return;
...
Ogre::Mesh* EC_ColladaMesh::PrepareMesh(const std::string& mesh_name, bool clone)
{
   /* Ogre::MeshPtr mesh;
    LogInfo(mesh_name);
    ...
    if (clone)
    {
        try
        {
            mesh = mesh->clone(renderer->GetUniqueObjectName("EC_ColladaMesh_clone"));
...
void EC_ColladaMesh::UpdateSignals()
{
...
void EC_ColladaMesh::OnAttributeUpdated(IAttribute *attribute)
{
...
void EC_ColladaMesh::OnComponentRemoved(IComponent* component, AttributeChange::Type change)
{
    if (component == placeable_.get())
        SetPlaceable(ComponentPtr());
}

void EC_ColladaMesh::OnMeshAssetLoaded(AssetPtr asset)
{
    ...
    if (mesh)
    {
        if (mesh->ogreMesh.get())
        {
            ...
            // 1. Ogre::Entity and Ogre::Mesh are valid (aka this is not the first load for this EC_ColladaMesh)
...
void EC_ColladaMesh::OnSkeletonAssetLoaded(AssetPtr asset)
{
...
void EC_ColladaMesh::ApplyMaterial()
{
    AssetReferenceList materialList = meshMaterial.Get();
...
bool EC_ColladaMesh::HasMaterialsChanged() const
{
...
bool EC_ColladaMesh::AttachMeshToBone(QObject* targetMesh, const QString& boneName)
{
    if (!entity_)
        return false;

    // First make sure that the target mesh is valid, and the bone can be found
    EC_ColladaMesh* targetMeshPtr = dynamic_cast<EC_ColladaMesh*>(targetMesh);
    if (!targetMeshPtr)
        return false;
...
void EC_ColladaMesh::DetachMeshFromBone()
{
